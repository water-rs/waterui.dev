...
const RoadmapSection = () => {
  const [activeVersion, setActiveVersion] = useState<
    (typeof roadmapMilestones)[number]["version"]
  >(roadmapMilestones[1].version);

  const selectedMilestone = useMemo(() => {
    const match = roadmapMilestones.find(
      (milestone) => milestone.version === activeVersion
    );
  ...
  const activeIndex = useMemo(() => {
    const index = roadmapMilestones.findIndex(
      (milestone) => milestone.version === activeVersion
    );
    ...
    <section className="grid gap-10 border border-water-black bg-water-ivory p-10">
      <header className="grid gap-3">
        ...
        <p className="max-w-2xl text-sm uppercase tracking-[0.25em] text-water-black/70">
          Built in public. Each release expands rendering depth, platform
          coverage, and tooling driven by the community.
        </p>
      </header>
      <div className="grid gap-10 lg:grid-cols-12 lg:items-start lg:gap-10">
        <article
          className={`grid gap-5 border border-water-black p-6 ${surface} lg:col-span-5`}
        >
          <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.35em]">
            <span className="border border-current px-3 py-1">
              {selectedMilestone.version}
            </span>
            <span>{selectedMilestone.status}</span>
          </div>
          <div
            className={`h-2 w-20 ${accentMarker[selectedMilestone.accent]}`}
            aria-hidden
          />
          ...
          <ul className="grid gap-2 text-xs uppercase tracking-[0.3em]">
            {selectedMilestone.highlights.map((highlight) => (
              <li
                key={highlight}
                className={`px-3 py-2 ${accentList[selectedMilestone.accent]}`}
              >
                {highlight}
              </li>
            ))}
                  ...
                  >
                    ...
                    <span
                      className={`text-[0.7rem] lowercase tracking-[0.05em] ${
                        isActive
                          ? accentSummary[milestone.accent]
                          : "text-water-black/70"
                      }`}
                    >
                      {milestone.summary}
